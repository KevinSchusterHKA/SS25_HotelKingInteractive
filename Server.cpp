...
void Server::SpielStarten() {
	...
	for (int i = 0; i < numberOfPlayer; i++) {
		...
		//menumanager.getGameFunctionManager().addPlayer(realPlayer);
		menumanager.getGameFunctionManager().getPlayers().push_back(realPlayer);
	...
	menumanager.getGameFunctionManager().setCurrentPlayer(2);	//1. Spieler beginnt
	menumanager.getGameFunctionManager().setCurrentRound(1);	//1. Runde

	//menumanager.getMenulog() << menumanager.getGameFunctionManager().getCurrentRound() << "ds" << endl;

	cout << "Current Player"<< menumanager.getGameFunctionManager().getCurrentPlayer() << endl;
...
void Server::SpielLaden() {
	...
	menumanager.setGameFunctionManager(gamefunctionmanager) ;

	cout << "Aktuelle Runde: " << menumanager.getGameFunctionManager().getCurrentRound() << endl;
	cout << "Aktueller Spieler: " << menumanager.getGameFunctionManager().getCurrentPlayer() << endl;

	for (const Player& p : menumanager.getGameFunctionManager().getPlayers()) {
		cout << "-----------------------------" << endl;
...
void Server::showIngameDialog() {
	...
	menumanager.getGameFunctionManager().setPlayers(players);
...
void Server::Spielzug() {
	int id = menumanager.getGameFunctionManager().getCurrentPlayer();
	cout << "id:" << id << endl;
		do {																			//Spielzug
			vector<int> dice = menumanager.getGameFunctionManager().rollDice();

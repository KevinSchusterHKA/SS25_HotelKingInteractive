...
void Server::SpielStarten() {
	...
	while (!config.loadConfig(rulesPath)) {
		...
		cin >> rulesPath; //selber eingeben, falls ung�ltig
	};
	...
	while (numberOfPlayer != 1 && numberOfPlayer != 2 && numberOfPlayer != 3 && numberOfPlayer != 4) {
		...
		cout << "ungueltige Eingabe \n \n Wie viele Spieler spielen? (1-4) " << endl;		//Anzahl der Spieler nochmal abfragen wenn ung�ltig
		cin >> numberOfPlayer;
	...
	//Player anzahlCPU = CPU(numberOfCPUPlayer); // Funktion in Player f�r CPU Gegner mit �bergabeparameter Anzahl CPU Gegner
...
////////////////////////////////////////////////////////////saveGame speichert nicht die aktuellen Spielst�nde ab sondern immer die gleichen voreingestellten////////////////////////////////////////////
...
void Server::Spielzug(GameFunctionManager& manager) {
	...
	cout << "id:" << id << endl;
	// Gefängnis-Logik prüfen
	//in Prison geht nicht, weil man kann nicht auf false setzen
	if (manager.getPlayers()[id].getPrisonCount() > 0) {
		cout << manager.getPlayers()[id].getName() << " ist im Gefängnis." << endl;

		vector<int> dice = manager.rollDice();
		cout << "Du hast gewürfelt: " << dice[0] << " und " << dice[1] << endl;

		if (dice[0] == dice[1]) {
			cout << "Du hast einen Pasch! Du kommst aus dem Gefängnis frei." << endl;
			manager.getPlayers()[id].setPrisonCount(0);  // Zähler zurücksetzen
			manager.getPlayers()[id].move(dice[0] + dice[1]);
		} else {
			manager.getPlayers()[id].deductPrisonTime();

			if (manager.getPlayers()[id].getPrisonCount() == 0) {
				cout << "Du hast deine Strafe abgesessen. Du kommst frei." << endl;
			} else {
				cout << "Kein Pasch. Du musst noch " << manager.getPlayers()[id].getPrisonCount() << " Runden im Gefängnis bleiben." << endl;
		...
		do {																			//Spielzug
			vector<int> dice = manager.rollDice();
			cout << "gew�rfelte Zahlen: " << dice[0] << ", " << dice[1] << endl;

			if (manager.getPaschCounter() == 3) {							//Wenn 3. Mal Pasch -> Gef�ngnis

				cout << "Das war dein 3. Pasch... Gehe ins Gef�ngnis" << endl;
				manager.getPlayers()[id].setPrison();
				manager.getPlayers()[id].setPosition(10);  // braucht man???

			}
			else {
				manager.getPlayers()[id].move(dice[0] + dice[1]); //move Summe aus gew�rfelten Zahlen
				dice.clear();
				int tile = manager.getPlayers()[id].getPosition();

				if (tile >= 40) {										//Checken ob �ber LOS gegangen
					tile = tile - 40;
					...
					if (i != id){
						...
						if (positionofothers == tile) {								//dieser Player steht schon auf dem Feld
							...
							if (manager.getPlayers()[id].getMoney() >= 50) {
								...
								cout << "Du hast aber leider nicht genug Geld daf�r... \nGame Over f�r dich :(";
								//Game Over
							}
						}
					}
				...
				if (auto propTile = dynamic_cast<PropertyTile*>(sTile.get())) {		//auf Property Tile
					...
					if (propTile->getOwnerId() == -1) {											//wenn Feld noch nicht gekauft
						if (manager.getPlayers()[id].getMoney() >= price) {									//wenn genug Geld zum Kaufen da
							cout << "M�chtest du das Feld kaufen?(dein Budget:"<<manager.getPlayers()[id].getMoney()<<") (0: nein, 1:ja)" << endl;
							cin >> buyfield;
							if (buyfield) {
						...
						if (manager.getPlayers()[id].getMoney() >= rent) {					//genug Geld f�r Rente

							manager.getPlayers()[id].addMoney(-rent);
							manager.getPlayers()[ownerid].addMoney(rent);
						}
						else {													//nicht genug Geld f�r Rente
							cout << "Du hast aber leider nicht genug Geld daf�r... \nGame Over f�r dich :(";
								//Game Over
						}
					}
					...
					if(specialtile == "Start") {
					...
					else if (specialtile == "GoToJail") {							//Ins Gef�ngnis gehen
						cout << "Gehe ins Gef�ngnis!" << endl;
						manager.getPlayers()[id].setPrison();
						manager.getPlayers()[id].setPosition(10);  // Gefängnisfeld
						...
						//Eventkarte ziehen und ggf ausf�hren
					}
					else if (specialtile == "Community") {							//Gemeinschaftsfeld
						//Gemeinschaftskarte ziehen und ggf ausf�hren
					}
						...
						if (manager.getPlayers()[id].getMoney() >= 100) {					//genug Geld zum Fliegen
							cout << "M�chtest du fliegen? (nein=0 ja=1)" << endl;
							cin >> transport;
						...
						if (manager.getPlayers()[id].getMoney() >= 50) {					//genug Geld zum Fliegen
							cout << "M�chtest du fahren? (nein=0 ja=1)" << endl;
							cin >> transport;
		...
		} while (manager.getPaschCounter() == 1 || manager.getPaschCounter() == 2);		//nochmal w�rfeln wenn du Pasch hattest

		//Spielerzug zuende -> n�chster Spieler
